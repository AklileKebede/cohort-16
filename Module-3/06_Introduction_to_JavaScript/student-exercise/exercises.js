...
*/
		...
		function lastDigit (){
			for (let i = 0; i < arguments.length-1; i++) {
				if (arguments[i] % 10 === arguments[i+1] % 10){
					return true;
				}
			}
			return false;
...
function frontAgain (givenString){
	let subString = givenString.substring(0,2);
	if (subString === givenString.substring(givenString.length-2)){
		return true;
	}  return false; 
...
*/
function cigarParty(cig,weekend){
	if (weekend === true && cig >=40){
		return true;
...
let filteredArrayByX=[];
for (let i = 0; i < array.length; i++) {
	if(array[i] % x === 0 ){
		filteredArrayByX.push(array[i]);
	} 
}
return filteredArrayByX;
}
/*
